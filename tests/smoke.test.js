...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Religious life")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/0e4485e1-3fb0-5d8c-a83c-5315bc85cb66')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/a98d0e50-3029-5377-a2cf-b09ed33dce7b')
    ...
    const isl = await mountSite('#/item/0e4485e1-3fb0-5d8c-a83c-5315bc85cb66')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '0e4485e1-3fb0-5d8c-a83c-5315bc85cb66')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Kiswa (tomb cover) for the Prophet of God, Ibrahim (Abraham)." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/825bbc31-2ea9-5983-a4b5-235679be04cb')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Greece")
    expect(host.textContent).toContain('Benaki Museum')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/04c60c4a-2430-5a48-ac2a-c1b3d488b403')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Benaki Museum'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Athens"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/04c60c4a-2430-5a48-ac2a-c1b3d488b403/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['religiousLife', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
