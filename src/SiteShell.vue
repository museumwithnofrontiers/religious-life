...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('religiousLife.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('religiousLife.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('religiousLife.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
